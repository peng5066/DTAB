...
class FastRCNNFocalLoss(FastRCNNOutputs):
    ...
    def losses(self):
        return {
            ...
            # "loss_cls": self.comput_qfocal_loss(),
            "loss_box_reg": self.box_reg_loss(),
        }

    # def comput_qfocal_loss(self):
    #     if self._no_instances:
    #         return 0.0 * self.pred_class_logits.sum()
    #     else:
    #         QFC_loss = QualityFocalLoss()
    #         # Calculate IOU between proposals and corresponding gt_box
    #         from detectron2.structures import Boxes, matched_pairwise_iou
    #         box = self.proposals
    #         gt_boxs = self.gt_boxes
    #         iou = matched_pairwise_iou(gt_boxs, box)
    #         total_loss = QFC_loss(pred=self.pred_class_logits, target=self.gt_classes, score=iou,
    #                               avg_factor=self.gt_classes.shape[0])
...
def quality_focal_loss(pred, label, score, weight=None,beta=2.0,
          reduction='mean',
          avg_factor=None):
    ...
    # QFL loss when all labels are 0, i.e., fill in the background loss first
    loss = F.binary_cross_entropy_with_logits(
        pred, zerolabel, reduction="none"
    ) * scale_factor.pow(beta)

    # FG cat_id: [0, num_classes -1], BG cat_id: num_classes
    bg_class_ind = pred.shape[1] - 1   # Index 20 is background
    # bg_class_ind = pred.size(1)
    # fg_inds = nonzero_tuple((label >= 0) & (label < bg_class_ind))[0]

    pos = torch.nonzero((label >= 0) & (label < bg_class_ind), as_tuple=False).squeeze(1)
    pos_label = label[pos].long()
    # positives are supervised by bbox quality (IoU) score
    scale_factor = score[pos] - pred_sigmoid[pos, pos_label]  # Use difference as weight factor, larger difference gives larger weight for online hard example mining
    loss[pos, pos_label] = F.binary_cross_entropy_with_logits(  # Cross-entropy loss for corresponding categories
        pred[pos, pos_label], score[pos], reduction="none"  # Category label is IOU value and is dynamic
